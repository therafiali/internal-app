...
const OperationsRedeemPage = () => {
  ...
  const [selectedRequest, setSelectedRequest] = useState<RedeemRequest | DisputedRequest | null>(null);
  ...
  useEffect(() => {
    const handleAutoOpenModal = () => {
      ...
      if (inProgressRequest) {
        ...
        } else if (inProgressRequest.processing_state.modal_type === "reject_modal") {
    ...
    if (user?.id && (redeemRequests.length > 0 || disputedRequests.length > 0)) {
      handleAutoOpenModal();
    }
  ...
  const isRedeemRequest = (request: RedeemRequest | DisputedRequest): request is RedeemRequest => {
    return 'redeem_id' in request && !('recharge_id' in request);
  };

  const isDisputedRequest = (request: RedeemRequest | DisputedRequest): request is DisputedRequest => {
    return 'recharge_id' in request;
  };

  const handleProcessClick = async (request: RedeemRequest | DisputedRequest) => {
    try {
      ...
      const { data: checkData, error: checkError } = await supabase
        .from(isDisputedRequest(request) ? "recharge_requests" : "redeem_requests")
      ...
      if (checkData?.processing_state?.status === "in_progress") {
        alert("This request is currently being processed by another user222222");
        return;
      }
      ...
      if (!data) {
        alert("Could not acquire processing state - request might be locked by another user");
        return;
      }
  ...
  const handleRejectClick = async (request: RedeemRequest | DisputedRequest) => {
    try {
      // Check if request is already in progress
      if (request.processing_state.status === "in_progress") {
        console.log("Request is already being processed by another user 555555555");
        return;
      }
    ...
    if (selectedRequest) {
      try {
        ...
        const { error } = await supabase
          .from(isDisputedRequest(selectedRequest) ? "recharge_requests" : "redeem_requests")
          .update({
            processing_state: {
              status: "idle",
              processed_by: null,
    ...
    return (
      <div className="flex gap-2">
        <button
          ...
          title={
            isInProgress
              ? `This request is being processed by ${userName}`
              : ""
          }
        >
          {isProcessLoading ? (
  ...
  const handleProcess = async (amount: string, notes: string) => {
    ...
    try {
      ...
      if (isDisputedRequest(selectedRequest)) {
        ...
        const amountToUpdate = selectedRequest.status === "verification_rejected" ? null : selectedRequest.total_amount;

        const { data, error } = await supabase.rpc("process_redeem_request_with_player_update", {
    ...
    try {
      ...
      if (isDisputedRequest(selectedRequest)) {
        ...
        try {
          await sendManyChatMessage({
            subscriberId: selectedRequest.messenger_id || '',
            message: MANYCHAT_TEMPLATES.REDEEM_REQUEST_REJECTED(selectedRequest.redeem_id),
            customFields: {
              redeem_request_id: selectedRequest.id,
            ...
            <tbody className="divide-y divide-gray-800">
              {redeemRequests.map((request, index) => (
                <tr key={index} className="hover:bg-[#252b3b]">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-300">
                    ${isDisputedRequest(request) ? request.amount : request.total_amount || 0}
                  </td>
                  {/* <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-300">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-3">
                      <div className="flex-shrink-0 h-8 w-8">
                        <Image
                          className="h-8 w-8 rounded-full object-cover border border-gray-700"
                          src={request.manychat_data?.profile?.profilePic || `https://ui-avatars.com/api/?name=${request?.player_name}`}
                          alt={`${request?.player_name}'s profile`}
                          width={32}
                          height={32}
                          onError={(e) => {
    ...
    <div className="flex min-h-screen bg-[#0a0a0a]">
      ...
      <div className="flex-1 pl-64">
        <main className="p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <RefreshButton onClick={handleRefresh} isLoading={isRefreshingRedeem || isRefreshingDisputed} />
          </div>
      ...
      {selectedRequest && (
        <ProcessModal
          ...
          redeemId={isDisputedRequest(selectedRequest) ? selectedRequest.recharge_id : selectedRequest.redeem_id}
          playerName={selectedRequest.player_name}
          playerData={selectedRequest.player_data || { profile: { profilePic: "" } }}
          init_by={selectedRequest.init_by}
          gameUsername={selectedRequest.game_username}
          platform={selectedRequest.game_platform}
          amount={isDisputedRequest(selectedRequest) ? selectedRequest.amount : selectedRequest.total_amount}
          bonus={0}
          promotion=""
          identifier=""
