...
CREATE OR REPLACE FUNCTION validate_assignment_availability_v2()
RETURNS TRIGGER AS $$
DECLARE
    v_redeem_id UUID;
    v_amount DECIMAL;
...
BEGIN
    ...
    -- Check if redeem request is not being processed
    IF v_redeem_action_status = 'in_progress' THEN
        RAISE EXCEPTION 'Redeem request % is already being processed', v_redeem_id;
    END IF;
    ...
    BEGIN
        -- Update the redeem request first
        UPDATE redeem_requests r
        SET 
            status = CASE 
                WHEN v_redeem_total = (v_new_hold_amount + v_redeem_paid) THEN 'hold_complete'
                ELSE 'hold_in_progress'
            END,
            amount_hold = v_new_hold_amount,
            action_status = 'in_progress'::action_status_type,
            updated_at = NOW()
        WHERE r.id = v_redeem_id;
...
CREATE TRIGGER validate_assignment_trigger
    ...
    EXECUTE FUNCTION validate_assignment_availability_v2();

-- Drop the old function if it exists and is not being used
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM pg_proc 
        WHERE proname = 'validate_assignment_availability'
    ) THEN
        DROP FUNCTION IF EXISTS validate_assignment_availability();
    END IF;
EXCEPTION WHEN OTHERS THEN
    -- If we can't drop it, that's fine
    NULL;
END $$; 