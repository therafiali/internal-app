...
drop function if exists assign_company_tag(uuid, numeric, text, uuid, text, text);

-- Create updated function to handle company tag assignment
create or replace function assign_company_tag(
  p_tag_id uuid,
  p_amount numeric,
  p_recharge_id text,
  p_user_email text,  -- Changed from uuid to text
  p_cashtag text,
  p_ct_type text
)
returns json
...
begin
  ...
  insert into assignment_redeem (
    ...
    tag_id,
    amount,
    assigned_by_email,  -- Changed column name to reflect email storage
    assigned_at,
    tag_type,
    cashtag,
    status
