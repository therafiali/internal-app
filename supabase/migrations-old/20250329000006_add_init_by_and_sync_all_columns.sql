-- Add init_by column to all tables if not exists
ALTER TABLE transactions
ADD COLUMN IF NOT EXISTS init_by UUID REFERENCES auth.users(id);
...
-- Update the recharge transaction handler function to include all columns
CREATE OR REPLACE FUNCTION handle_recharge_request_transaction()
RETURNS TRIGGER AS $$
BEGIN
    -- Insert a new transaction record with all columns
...
-- Update the redeem transaction handler function to include all columns
...
-- Update the recharge sync function to include all columns
...
-- Update the redeem sync function to include all columns
...
-- Function to sync existing init_by values
CREATE OR REPLACE FUNCTION sync_existing_init_by()
RETURNS void AS $$
BEGIN
    -- Update transactions from recharge_requests
    UPDATE transactions t
    SET init_by = COALESCE(r.init_by, t.init_by)
    FROM recharge_requests r
    WHERE t.recharge_id = r.recharge_id
    AND t.recharge_uuid = r.id
    AND t.init_by IS NULL;
    ...
    -- Set default init_by for remaining NULL values to the first admin user
    UPDATE transactions
    SET init_by = (
        SELECT id 
        FROM users 
    ...
    WHERE init_by IS NULL;

    RAISE NOTICE 'Updated all init_by values';
END;
$$ LANGUAGE plpgsql;

-- Execute the sync function
SELECT sync_existing_init_by();

-- Add rollback function
CREATE OR REPLACE FUNCTION rollback_init_by_changes()
RETURNS void AS $$
BEGIN
    -- Remove init_by columns
    ALTER TABLE transactions DROP COLUMN IF EXISTS init_by;
    ALTER TABLE recharge_requests DROP COLUMN IF EXISTS init_by;
    ALTER TABLE redeem_requests DROP COLUMN IF EXISTS init_by;
END;
$$ LANGUAGE plpgsql; 